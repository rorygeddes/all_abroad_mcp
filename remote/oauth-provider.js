...
//
// Implements OAuthServerProvider backed by the All Abroad Supabase project.
// Handles the full OAuth 2.1 / PKCE flow so any All Abroad user can authenticate
...
if (!SUPABASE_URL || !SERVICE_ROLE_KEY || !ANON_KEY) {
  throw new Error(
    "[allabroad-mcp] Missing required env vars: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SUPABASE_ANON_KEY"
  );
}
...
class SupabaseClientsStore {
  async getClient(clientId) {
    ...
    return {
      ...
      scope:                      data.scope ?? "allabroad:read",
      ...
      .upsert(
        {
          ...
          scope:                      client.scope ?? "allabroad:read",
...
// ── AllAbroadOAuthProvider ────────────────────────────────────────────────────

export class AllAbroadOAuthProvider {
  constructor() {
  ...
  // Step 2a: Retrieve the stored PKCE challenge for a code.
  ...
  async exchangeAuthorizationCode(client, authorizationCode, codeVerifier, redirectUri) {
    ...
    return {
      ...
      refresh_token: refreshToken,
      scope:         session.scope ?? "allabroad:read",
    };
  }
  ...
  async verifyAccessToken(token) {
    ...
    return {
      ...
      scopes:    (session.scope ?? "allabroad:read").split(" "),
...
export async function getSupabaseJwtForMcpToken(mcpToken) {
  ...
  if (!session.supabase_refresh_token) {
    throw new Error("No Supabase session linked — user must re-authenticate");
  }
  ...
  if (refreshError || !refreshed?.session) {
    throw new Error("Supabase session expired. User must reconnect All Abroad in Claude settings.");
  }
